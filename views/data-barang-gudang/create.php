<?php
    ...
    $result=mysqli_query($conn, "SELECT COUNT(idBarang) + 1 AS id from gudang");
    $query=mysqli_fetch_assoc($result);
    
    if(isset($_POST["submit"])){
            if(createBarangGudang($_POST) > 0 ){
                echo "
                    <script>
...
<head>
    ...
    <title>Inventori Barang | Create Data Barang Gudang</title>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-dark">Tambah Barang Gudang</h6>
            </div>
            <div class="card-body">
            <form method="POST" action="">
            
                <div class="row mb-3">
                    <label for="kodeBarang" class="col-md-4 col-form-label text-md-end">Kode Barang</label>
            
                    <div class="col-md-6">
                        <input id="kodeBarang" type="hidden" name="kodeBarang" value="<?php if( $query['id'] <= 9 ) : ?>GDG-00000<?= $query['id'] ?><?php elseif( $query['id'] <= 99 ) : ?>GDG-0000<?= $query['id'] ?><?php elseif( $query['id'] <= 999 ) : ?>GDG-000<?= $query['id'] ?><?php elseif( $query['id'] <= 9999 ) : ?>GDG-00<?= $query['id'] ?><?php elseif( $query['id'] <= 99999 ) : ?>GDG-0<?= $query['id'] ?><?php elseif( $query['id'] <= 999999 ) : ?>GDG-<?= $query['id'] ?><?php else : ?>NULL<?php endif; ?>" required>
                            
                        <input type="text" class="form-control" value="<?php if( $query['id'] <= 9 ) : ?>GDG-00000<?= $query['id'] ?><?php elseif( $query['id'] <= 99 ) : ?>GDG-0000<?= $query['id'] ?><?php elseif( $query['id'] <= 999 ) : ?>GDG-000<?= $query['id'] ?><?php elseif( $query['id'] <= 9999 ) : ?>GDG-00<?= $query['id'] ?><?php elseif( $query['id'] <= 99999 ) : ?>GDG-0<?= $query['id'] ?><?php elseif( $query['id'] <= 999999 ) : ?>GDG-<?= $query['id'] ?><?php else : ?>NULL<?php endif; ?>" disabled>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <input id="namaBarang" type="text" class="form-control" name="namaBarang" required placeholder="Masukkan Nama Barang">
                    </div>
                </div>
                
                <div class="row mb-3">
                    <label for="jenisBarang" class="col-md-4 col-form-label text-md-end">Jenis Barang</label>
                ...
                <input id="stok" type="hidden" class="form-control" name="stok" value=0 required>
